...
const ExamList = () => {
    const [currentPage, setCurrentPage] = useState(1);
      ...
      const tableData = Array.from({ length: 50 }, (_, index) => [
        ...
        `CIA.No${String(index + 1).padStart(3, "0")}`,
        `Course${index + 1}`,
        `210${index + 1}`,
        `Religion ${index + 1}`,
        `Email Id ${index + 1}`,
      ...
      const filteredData = tableData.filter((row) => {
        ...
        const matchesCategory = selectedCategory === "All" || row[14] === selectedCategory;
        // console.log(matchesCategory)
        const matchesCollege = selectedCollege === "All" || row[13] === selectedCollege;
  ...
  <CustomButton children={"Add Final Exam"} buttonStyle={styles.buttonStyle} buttonStyleText={styles.buttonStyleText} onClick={()=>router.push('/screens/Admin/AddFinalExam')} />
</View>
      </View>
