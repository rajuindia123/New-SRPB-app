...
import { getDocuments, getDocumentsAllDataTwo } from "../../Function/AppwriteCollection";
...
const AdmissionList = () => {
  ...
  const tableData1 = allStudents.length > 0 ? allStudents.sort((a, b) => new Date(b.$createdAt) - new Date(a.$createdAt)).map(student => [
    student.stuUAN || "",
    student.stuRankNo || "",
    student.stuName || "",
  ...
  useEffect(() => {
    const handelAllGetStudent = async () => {
      setLoading(true);
      try {
        const getAllStudent = await getDocumentsAllDataTwo(EnrollmentStudentsCollectionId,AdmissionCollectionId);
          ...
          <View style={styles.container}>
            ...
            <View style={{ justifyContent: 'flex-end', flexDirection: 'row' }}>
              <View style={{ width: '50%' }}>
                ...
                <CustomButton children={"Add Admission"} buttonStyle={styles.buttonStyle} buttonStyleText={styles.buttonStyleText} onClick={() => router.push('/screens/Admin/AddAdmission')} />
              </View>
            </View>
